...
(* color list *)
let all_colors = ["red"; "blue"; "orange"; "pink";
                  "black"; "yellow"; "green"; 
...
(** swap_out] removes and returns card with id [id] from the pile of
    player [p]*)
...
and  forced_deal board =
  ...
  let id = read_line () in if id = "cancel" then false 
    ...
    let card_out = swap_out currpl id in print_endline "Enter enter the name of 
    the person you want to swap \ properties with. The players are: \027[0m";
...
let debt_collector board = 
  ...
  if name = "cancel" then false else
    (let player = get_player_from_name board name in
     print_pile_of_player board name;
     ask_for_money board (List.nth (get_players board) 
                            (get_current_turn board)) player 5 0;
     true)
...
let rec deal_breaker board = 
  ...
  if name = "cancel" then false 
    ...
    let rec get_color () = 
      match read_line () with
      | s when String.length s > 0 && s <: all_colors -> s
...
let rec rent_helper board id = 
  ...
  let currpl = List.nth (get_players board) (get_current_turn board) in
  let colors = try 
      match (List.find (fun c -> get_id c = id) (get_cards_in_hand currpl)) with
      | Rent r -> get_rent_colors r
      | _ -> failwith "precondition violated." 
    with _ -> raise InvalidCard in

  let color_string =
    if colors <> [] then
      List.fold_left (fun acc color -> acc ^ color ^ "\n") "" colors 
    else
      "You can ask for rent for any color!" in

  print_endline color_string; 

  let rec ask_for_color () = 
    match read_line () with
    | s when (List.mem s colors || colors = [] && List.mem s all_colors || s = "cancel") -> s
    | _ -> print_endline "this isnt' a valid color!"; ask_for_color () in
...
and main_helper (board: board) (num: int) = 
  (* let currpl = List.nth (get_players board) (get_current_turn board) in *)
